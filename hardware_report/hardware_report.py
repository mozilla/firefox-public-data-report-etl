...
def load_data(spark, date_from, date_to):
  ...
  hardware_by_dimensions_df = spark.read.format('bigquery') \
...
def to_dict(row):
  ...
  return {
    ...
    "gfx0_model": get_device_family_chipset(row["gfx0_vendor_id"], row["gfx0_device_id"], DEVICE_MAP),
  }
def add_counts(dict):
...
def collapse_buckets(aggregated_data, count_threshold, sample_count):
  ...
  for dimension, counts in collapsed_groups.items():
    ratios[dimension] = {str(metric): count/float(sample_count) for metric, count in counts.items()}
...
def upload_data_s3(spark, bq_table_name, s3_bucket, s3_path):
  hardware_aggregates_df = spark.read.format('bigquery') \
        .option('table', bq_table_name) \
        .load()

  map_fields = ["arch", "cpu_cores", "cpu_vendor", "cpu_speed", "gfx0_vendor_name", "gfx0_model",
  ...
  aggregates_flattened = sorted(flatten_aggregates(aggregates), key=lambda a: a['date'], reverse=True)
  ...
  transfer.upload_file("hwsurvey-weekly.json", s3_bucket, s3_path+archived_file_copy)
  transfer.upload_file("hwsurvey-weekly.json", s3_bucket, s3_path+"hwsurvey-weekly.json")

date_type = click.DateTime()
@click.command()
@click.option(
  "--date_from", type=date_type, required=True, help="Aggregation start date (e.g. yyyy-mm-dd)"
)
@click.option(
  "--bq_table", required=True, help="Output BigQuery table"
)
@click.option(
  "--s3_bucket", required=True, help="S3 bucket for storing data"
)
@click.option(
  "--s3_path", required=True, help="S3 path for storing data"
)
@click.option(
  "--past_weeks", type=int, default=0, help="Number of past weeks to include (useful for backfills)"
)
def main(date_from, bq_table, s3_bucket, s3_path, past_weeks):
  ...
  for batch_number in range(0, past_weeks+1):
    ...
    logger.info(f"Running batch {batch_number}/{past_weeks}, timeframe: [{batch_date_from}, {batch_date_to})")
    hardware_by_dimensions_df = load_data(spark, batch_date_from, batch_date_to)

    aggregated = hardware_by_dimensions_df.rdd.map(to_dict).map(add_counts).fold({},combine)
    ...
    aggregates_df.write.format('bigquery') \
