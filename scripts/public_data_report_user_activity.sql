...
new_profile_rate AS (
    SELECT
        country_name,
        SAFE_DIVIDE(100 * countif(`moz-fx-data-shared-prod.udf.pos_of_trailing_set_bit`(days_created_profile_bits)<7), -- new profiles
...
FROM
    mau
JOIN
    daily_usage USING (week_start, country_name)
JOIN
    intensity  USING (week_start, country_name)
JOIN
    new_profile_rate USING (week_start, country_name)
JOIN
    latest_version_ratio USING (week_start, country_name)
WHERE
    mau.week_start NOT IN ('2019-04-29', '2019-05-06') --armagaddon: https://blog.mozilla.org/blog/2019/05/09/what-we-do-when-things-go-wrong/
ORDER BY
    week_start, country_name
